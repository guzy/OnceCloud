			...
			if (array.length >= 1) {
				...
				for (var i = 1; i < array.length; i++) {
					...
					if (imagestatus == 1) {
						if (i == 1) {
							tableStr = tableStr
									+ '<div class="image-item selected" imageid="'
									+ imageid + '" platform="' + imageplatform
									+ '">' + imagename + '</div>';
...
$('#createvmAction').on('click', function(event) {
	...
	if (valid && pwval) {
		...
		for (var i = 0; i < vmCount; i++) {
			var vmuuid = uuid.v4();
			preCreateVM(vmuuid, imageuuid, cpuCore, memoryCapacity, vmName,
					loginPwd);
		}
...
function preCreateVM(vmuuid, imageuuid, cpuCore1, memoryCapacity, vmName,
	...
	$("#tablebody")
			.prepend('<tr rowid="'
					...
					+ memoryStr
					+ '</td><td name="mac"></td><td name="plateform"></td><td name="groupName">'+'默认分组'+'</td><td name="backuptime">'
					+ backupStr
